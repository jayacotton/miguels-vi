...
/* Get filename, adding filetype if missing
   ----------------------------------------
*/
void get_fname(dest, fname, ftype)
...
void read_com()
{
	...
	if((com_size = st.st_blocks*st.st_blksize) == -1) {
		err_read();
	}
	...
	for(i = 0; i < com_size; ++i) {
		if(!strcmp((char *)com_buf + i, "TE_CONF")) {
			com_cf = i;
			break;
		}
	}
	...
	p = com_buf + com_cf + strlen((char *)com_buf + com_cf) + 1;
