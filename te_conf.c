...
_cf_num:       	defb 4                ;  1 byte  > Number of digits for line numbers (see cf_mx_lines) plus 1
...
_cf_list_chr:  	defb "-*>",0,0,0,0,0  ;  8 byte  > Legal characters for automatic lists + ZERO.
_cf_rul_chr:   	defb '.'              ;  1 byte  > Ruler: Character.
_cf_rul_tab:   	defb '!'              ;  1 byte  > Ruler: Tab stop character.
_cf_vert_chr:  	defb '|'              ;  1 byte  > Char. for vertical draws -- ie: for line numbers.
_cf_horz_chr:  	defb '-'              ;  1 byte  > Char. for horizontal draws -- ie: for status line.
_cf_lnum_chr:  	defb '|'              ;  1 byte  > Char. between line numbers and text.
...
_cf_keys:      	defb 0,0,0,0,0,0,0,0  ; 29 bytes > 1st character in key binding. Must to be equal to KEYS_MAX.
		defb 0,0,0,0,0,0,0,0
		defb 0,0,0,0,0,0,0,0
		defb 0,0,0,0,0
_cf_keys_ex:   	defb 0,0,0,0,0,0,0,0  ; 29 bytes > 2nd character in key binding. Must to be equal to KEYS_MAX.
		defb 0,0,0,0,0,0,0,0
		defb 0,0,0,0,0,0,0,0
		defb 0,0,0,0,0
_cf_bytes:     	defw $ - _cf_start + 2 ;  2 bytes > Block configuration size in bytes.
#endasm
